...
const getKeyFrames = (
  ...
  for (let i = 1; i < steps; i++) {
    ...
    const shadowX = random(-2, 2);
    const shadowY = random(-2, 2);

    keyframes.push({
      keys: [p + tick, p + glitchPercentageDuration - tick],
      css: {
        transform: `translateX(var(--glitch-x-${i}))`,
        filter: `hue-rotate(var(--glitch-hue-${i})) drop-shadow(${shadowX}em ${shadowY}em 0 ${color})`,
      },
    });
  }
...
export const generateGlitch = () => {
  const html = getGlitchHTML(31);
